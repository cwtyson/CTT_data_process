ml_localize_dets_error_fn <- function(file_f,
                                      output_folder,
                                      grid_points,
                                      failure_date_file,
                                      log_dist_RSSI_mdl,
  ...
  ## Focal band
  band_f = unlist(strsplit(file_f, split = "/"))[8]
  ...
  ## Get template of detections to count the number of columns
  dets_c <- readr::read_csv(file_f,show_col_types = FALSE)
  
  ## Column types:
  col_types = paste0("ccT", paste(rep('d', ncol(dets_c)-4), collapse=''),"T")
  ...
  ## Check if there is a failure date for tag
  if(!is.na(failure_date_file)){
    
    failure_dates <- read_csv(failure_date_file,show_col_types = FALSE)  %>% 
      filter(tag == band_f) %>% 
      pull(fail_date)
  ...
  if(exists("dets_p")){
    ...
    ## Focal band dates
    band_f_date = gsub(".csv.gz","",
                       unlist(strsplit(file_f, split = "/"))[9])
